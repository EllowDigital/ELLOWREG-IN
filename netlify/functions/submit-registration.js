...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { name, phone, firmName, address, district, state, attendance } =
      fields;
    ...
    if (!profileImage)
      return {
        statusCode: 400,
        body: JSON.stringify({ error: "Profile photo is required." }),
      };
    if (!paymentScreenshot)
      return {
        statusCode: 400,
        body: JSON.stringify({ error: "Payment screenshot is required." }),
      };
